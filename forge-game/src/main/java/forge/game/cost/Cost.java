...
public class Cost implements Serializable {
    ...
    private static CostPart parseCostPart(String parse, boolean tapCost, boolean untapCost) {
        ...
        if (parse.startsWith("FlavorAction<")) {
            ...
            final String[] splitStr = abCostParse(parse, 2);
            return new CostFlavorAction(splitStr[0], splitStr[1]);
        }
        ...
        // wish effects reach the sideboard; this one reaches your actual collection
        // (Collector Protector)
        if (parse.startsWith("ExileFromSideboard<")) {
            final String[] splitStr = abCostParse(parse, 3);
            final String description = splitStr.length > 2 ? splitStr[2] : null;
            return new CostExile(splitStr[0], splitStr[1], description, ZoneType.Sideboard);
        }

        if (parse.startsWith("GiveFromCollection<")) {
            final String[] splitStr = abCostParse(parse, 3);
            final String description = splitStr.length > 2 ? splitStr[2] : null;
            return new CostGiveFromCollection(splitStr[0], splitStr[1], description);
        }
        ...
        if (parse.startsWith("RevealFromLibrary<")) {
            final String[] splitStr = abCostParse(parse, 3);
            final String description = splitStr.length > 2 ? splitStr[2] : null;
            return new CostReveal(splitStr[0], splitStr[1], description, "Library");
        }
