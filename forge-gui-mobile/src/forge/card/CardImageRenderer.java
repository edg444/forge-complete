        ...
        if (image == null || image == ImageCache.getInstance().getDefaultImage() || (Forge.enableUIMask.equals("Art") || card.useCardArt())) { //support drawing card image manually if card image not found
            ...
            if (canshow && displayFlipped) {
                ...
                if (Forge.enableUIMask.equals("Full")) {
                    g.drawCardRoundRect(image, f_x, f_y, f_w, f_h, f_x + f_w / 2, f_y + f_h / 2, 180, 1f, CardRendererUtils.getFoilIndex(card));
                } else if (Forge.enableUIMask.equals("Crop")) {
                    g.drawCardRoundRect(ImageCache.getInstance().croppedBorderImage(image), f_x, f_y, f_w, f_h, f_x + f_w / 2, f_y + f_h / 2, 180, 0f, CardRendererUtils.getFoilIndex(card));
                } else {
                    g.drawCardRoundRect(image, f_x, f_y, f_w, f_h, f_x + f_w / 2, f_y + f_h / 2, 180, 0f, CardRendererUtils.getFoilIndex(card));
                }
